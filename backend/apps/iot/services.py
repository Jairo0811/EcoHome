...
def mqtt_configuration() -> dict:
    return {"host": os.getenv("MQTT_HOST", "localhost"), "port": int(os.getenv("MQTT_PORT", "1883")), "tls": os.getenv("MQTT_TLS", "false").lower() == "true", "username": os.getenv("MQTT_USERNAME", "")}
...
def parse_payload(raw: str | bytes) -> dict:
    ...
    if not isinstance(value, dict): raise ValueError("El payload MQTT debe ser un objeto JSON.")
    return value
...
def ingest_message(external_id: str, payload: dict) -> IngestResult:
    ...
    if status not in Device.Status.values: status = Device.Status.WARNING
    ...
    if metric in Telemetry.Metric.values and value is not None:
        telemetry = Telemetry.objects.create(device=device, metric=metric, value=value, unit=payload.get("unit", ""), recorded_at=payload.get("recorded_at") or timezone.now())
        try:
            from apps.automation.services import evaluate_home
            evaluate_home(device.home)
        except Exception:
            logger.exception("No se pudieron evaluar alertas/automatizaciones para el hogar %s", device.home_id)
