...
export function AdvancedAnalytics() {
  ...
  return (
    <article className="panel consumption-panel" id="reportes">
      <div className="panel-heading">
        <div><p className="eyebrow">Analítica · 30 días</p><h3>Rendimiento del hogar</h3></div>
        <span className="period-chip">{data.periodDays} días</span>
      </div>

      <div className="stats-grid">
        <div><strong>{data.alerts.open}</strong><span className="subtitle"> alertas abiertas</span></div>
        <div><strong>{data.securityEvents}</strong><span className="subtitle"> eventos seguridad</span></div>
        <div><strong>{data.automationExecutions}</strong><span className="subtitle"> automatizaciones</span></div>
      </div>
            ...
            {energy.map((point) => (
              <span
                ...
                style={{
                  ...
                  flex: 1,
                  minWidth: 4,
                  height: `${Math.max((point.total / peak) * 100, 4)}%`,
